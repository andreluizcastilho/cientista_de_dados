...
# A - Histograma do preco
hist(dados$Preco)

# B - Boxplot do preco
boxplot(dados$Preco)

# C - Estatisticas descritivas do preco
summary(dados$Preco)
...
# A - Boxplot entre o Preco e Quadrimestre
boxplot(dados$Preco~ dados$Quadrimestre)

# B - Realiza analise de variancia
anova <- aov(Preco ~ Quadrimestre, data = dados)
summary(anova)

########
# 3 - Explore a relação entre as variáveis Preço e Portas, responda:
...
########
# 4 - Explore a relação entre as variáveis Preço e Quilometragem, responda:

plot(y = dados$Preco ,
     x = dados$Quilometragem,
     pch = 16)

# A - Coeficiente de correlacao
cor(dados$Preco, dados$Quilometragem)

# B - Ajuste regressao linear do Preco em funcao da Quilometragem
regressao_linear <- lm(Preco ~ Quilometragem, data = dados)
summary(regressao_linear)

# C - Analise descritiva da variavel quilometragem
summary(dados$Quilometragem)

# D - Coeficiente de variacao
sd(dados$Quilometragem) / mean(dados$Quilometragem)


########
# 5 - Explore a correlacao entre o Dolar e o Preco
plot(y = dados$Preco,
     x = dados$Dolar,
     pch = 16)
